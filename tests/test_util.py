"""Test util.py in BLE Radar."""

from __future__ import annotations

from math import floor

# Aggiornamento del percorso del modulo
from custom_components.ble_radar import util


def test_mac_math_offset():
    """Verifica il calcolo degli offset sugli indirizzi MAC."""
...
def test_mac_norm():
    """Verifica la normalizzazione dei vari formati di MAC address."""
...
def test_mac_explode_formats():
    """Verifica la generazione di formati alternativi (trattini, due punti, ecc)."""
...
def test_mac_redact():
    """Verifica l'offuscamento dei MAC per la privacy nei log."""
...
def test_rssi_to_metres():
    """Verifica la formula di conversione da RSSI a metri (Calcolo Fallback)."""
    assert floor(util.rssi_to_metres(-50, -20, 2)) == 31
    assert floor(util.rssi_to_metres(-80, -20, 2)) == 1000


def test_clean_charbuf():
    """Verifica la pulizia dei buffer di caratteri (rimozione null byte)."""
